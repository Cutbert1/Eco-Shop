...
def update_basket_quantity(request, item_id):
    ...
    if allowed_quantity <= 0:
        ...
        messages.warning(
            request,
            f'{product.name} was removed because the requested quantity is not available.', # noqa
        )
        return redirect(reverse('view_basket'))
...
def validate_quantity_against_stock(request, product, quantity_requested, current_quantity): # noqa
    ...
    if desired_total > max_quantity:
        ...
        messages.warning(
            request,
            f'Only {allowed} more of {product.name} can be added to your basket.', # noqa
        )
        return allowed
